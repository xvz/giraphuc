...
public class SemimetricTriangles  {
  ...
  /**
   * Indicates whether semi-metric edges will be
   * removed in the output graph.
   */
  ...
  public static class FindSemimetricEdges extends AbstractComputation<
    LongWritable, Writable, DoubleWritable, SimpleEdge, WritableComparable> {

    /** */
    private boolean removeEdgesEnabled;
    ...
    @Override
    public void compute(Vertex<LongWritable, Writable, DoubleWritable> vertex,
      Iterable<SimpleEdge> messages) throws IOException {
      ...
      for (SimpleEdge msg : messages) {
        ...
        if (vertex.getEdgeValue(id1) != null) {
          ...
          double weightXab = vertex.getEdgeValue(id2).get();
          double weightXac = vertex.getEdgeValue(id1).get();
          double weightXbc = msg.getWeight();

          if (weightXab + weightXac < weightXbc) {
            if (removeEdgesEnabled) {
              removeEdgesRequest(id1, id2);
              removeEdgesRequest(id2, id1);
            }
          } else if (weightXab + weightXbc < weightXac) {
            if (removeEdgesEnabled) {
              removeEdgesRequest(vertex.getId(), id1);
              removeEdgesRequest(id1, vertex.getId());
            }
          } else if (weightXbc + weightXac < weightXab) {
            if (removeEdgesEnabled) {
  ...
  public static class Finalize extends AbstractComputation<LongWritable,
    ...
    public void compute(Vertex<LongWritable,
        NullWritable, DoubleWritable> vertex,
        Iterable<Writable> messages) throws IOException {
      vertex.voteToHalt();
    }
  }
