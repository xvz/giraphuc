...
import org.apache.giraph.examples.MSTAsyncComputation.MSTVertexValue;
import org.apache.giraph.examples.MSTAsyncComputation.MSTEdgeValue;
...
/**
 * Distributed MST implementation, adapted for async execution.
 *
 ...
 */
@Algorithm(
    name = "Minimum spanning tree (async)"
)
public class MSTAsyncComputation extends BasicComputation<
    LongWritable, MSTVertexValue, MSTEdgeValue, MSTMessage> {
  ...
  private static String SUPERVERTEX_AGG = "supervertex_PERSIST";
  /** Computation phase aggregator name */
  private static String PHASE_AGG = "phase_PERSIST";
  ...
  private void phase1(
    ...
    long eId = vertex.getId().get();
    MSTEdgeValue eVal = null;

    // find minimum weight edge
    for (Edge<LongWritable, MSTEdgeValue> e : vertex.getEdges()) {
    ...
    if (minEdge != null) {
      vertex.getValue().setWeight(minEdge.getWeight());
      vertex.getValue().setSrc(minEdge.getSrc());
      vertex.getValue().setDst(minEdge.getDst());
    } else {
      // this is an error
      LOG.error("No minimum edge for " + vertex.getId() + " found in PHASE_1.");
    }

    // technically part of PHASE_2A
    vertex.getValue().setPointer(minId);

    // go to phase 2A

    //LOG.info(vertex.getId() + ": min edge is " + minEdge +
    //         " and value is " + vertex.getValue());
  }
  ...
  private void phase2A(
    ...
    sendMessage(new LongWritable(vertex.getValue().getPointer()), msg);
  ...
  private void phase2B(
    ...
    for (MSTMessage message : messages) {
      ...
      case MSTMsgType.MSG_QUESTION:
        ...
        //LOG.info(vertex.getId() + ": received question from " + senderId);
        ...
        if (senderId == pointer) {
          ...
          if (myId <= senderId) {
            ...
            //LOG.info(vertex.getId() + ": I am a supervertex");
          } else {
            type = MSTVertexType.TYPE_POINTS_AT_SUPERVERTEX;
            //LOG.info(vertex.getId() + ": I point to supervertex" + senderId);
          }
      ...
      case MSTMsgType.MSG_ANSWER:
        ...
        if (type != MSTVertexType.TYPE_UNKNOWN) {
          //LOG.info(vertex.getId() + ": ignoring answers");
          break;
        }
        ...
        //LOG.info(vertex.getId() + ": received answer from " +
    ...
    // update vertex value
    vertex.getValue().setType(type);
    vertex.getValue().setPointer(pointer);
  ...
  private void phase3B(
    ...
    for (MSTMessage message : messages) {
      ...
      case MSTMsgType.MSG_CLEAN:
        ...
        //LOG.info(vertex.getId() + ": received MSG_CLEAN from " + senderId);
        ...
        if (supervertexId == pointer) {
          vertex.removeEdges(new LongWritable(senderId));
          ...
          eTmp = vertex.getEdgeValue(new LongWritable(senderId));
          if (eTmp == null) {
          ...
          eValExisting = vertex.getEdgeValue(new LongWritable(supervertexId));
          ...
          if (eValExisting == null) {
            ...
            vertex.addEdge(EdgeFactory.create(new LongWritable(supervertexId),
                                              eVal));
            ...
            if (eVal.getWeight() < eValExisting.getWeight()) {
              vertex.setEdgeValue(new LongWritable(supervertexId), eVal);
            }
          }
    ...
    //for (Edge<LongWritable, MSTEdgeValue> edge : vertex.getEdges()) {
    //  LOG.info(vertex.getId() + ": after 3B...edge to " +
    //           edge.getTargetVertexId() + " with " + edge.getValue());
    //}
  ...
  private void phase4B(
    ...
    for (MSTMessage message : messages) {
      ...
      case MSTMsgType.MSG_EDGE:
        ...
        eVal = e.getValue();
        eValExisting = vertex.getEdgeValue(eId);
...
//  /**
//   * Worker context used with {@link MSTAsyncComputation}.
//   * For debugging purposes only.
//   */
//  public static class MSTAsyncWorkerContext extends
//      WorkerContext {
//
//    @Override
...
//  }
  ...
  /**
   * Master compute associated with {@link MSTAsyncComputation}.
   * It registers required aggregators.
   */
  public static class MSTAsyncMasterCompute extends
    ...
    public void compute() {
      ...
      if (getSuperstep() == 0) {
        setNextPhase(MSTPhase.PHASE_1.get());
   ...
   * {@link MSTAsyncComputation}
   */
  public static class MSTAsyncVertexOutputFormat extends
      TextVertexOutputFormat<LongWritable, MSTVertexValue, MSTEdgeValue> {
    @Override
    public TextVertexWriter createVertexWriter(TaskAttemptContext context)
      throws IOException, InterruptedException {
      return new MSTAsyncVertexWriter();
    }

    /**
     * Simple VertexWriter that supports {@link MSTAsyncComputation}
     */
    public class MSTAsyncVertexWriter extends TextVertexWriter {
      @Override
  ...
  /**
   * Vertex value type used by {@link MSTAsyncComputation}.
   */
  ...
  /**
   * Edge value type used by {@link MSTAsyncComputation}.
   */
  ...
  /**
   * Message type used by {@link MSTAsyncComputation}.
   *
