...
for ((i = 0; i <= ${_NUM_MACHINES}; i++)); do
    ...
    ssh ${_MACHINES[$i]} "cd \"$dir\"; sar 1 > ./logs/${cpufile} & free -s 1 | stdbuf -o0 grep + > ./logs/${memfile} & sar -n DEV 1 | stdbuf -o0 grep 'lo\|eth0' > ./logs/${netfile} & cat /proc/net/dev > ./logs/${nbtfile}" &
done
wait