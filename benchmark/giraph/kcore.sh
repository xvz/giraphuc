...
if [ $# -ne 4 ]; then
    echo "usage: $0 input-graph machines exec-mode k-value"
    echo ""
...
execmode=$3
case ${execmode} in
    0) execopt="";;      # sync BSP is used by default
...
## log names
logname=kcore_${inputgraph}_${machines}_${execmode}_${kval}_"$(date +%Y%m%d-%H%M%S)"
...
hadoop jar "$GIRAPH_DIR"/giraph-examples/target/giraph-examples-1.1.0-for-hadoop-1.0.4-jar-with-dependencies.jar org.apache.giraph.GiraphRunner \
    ...
    org.apache.giraph.examples.okapi.graphs.KCore\$KCoreComputation \
    -ca core.k=${kval} \
    -ca giraph.vertexResolverClass=org.apache.giraph.examples.okapi.graphs.KCore\$KCoreVertexResolver \
    -vif org.apache.giraph.examples.okapi.io.formats.KCoreInputFormat \
    -vip /user/${USER}/input/${inputgraph} \
    -vof org.apache.giraph.examples.okapi.io.formats.KCoreOutputFormat \
    -op "$outputdir" \
