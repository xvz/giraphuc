...
## log names
logname=dprtolfinder_${inputgraph}_${machines}_0_"$(date +%Y%m%d-%H%M%S)"
...
for ((i = 1; i <= ${_NUM_MACHINES}; i++)); do
    darray[${i}]=$(ssh ${_MACHINES[$i]} "cat \"$HADOOP_DIR\"/logs/userlogs/${jobid}/*/syslog | grep 'max change' | awk '{print \$9}' | tr '\n' ','")
done
