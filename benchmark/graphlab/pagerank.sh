...
if [ $# -ne 4 ]; then
    echo "usage: $0 input-graph machines engine-mode supersteps/tolerance"
    echo ""
    echo "engine-mode: 0 for synchronous engine (supersteps)"
    echo "             1 for asynchronous engine (tolerance)"
    exit -1
fi
