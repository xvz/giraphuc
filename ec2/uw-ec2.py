...
def get_args():
    ...
    parser.add_argument('action', choices=ACTIONS,
                        help="R|Action to perform: \n"
                             ...
                             "            for all systems (giraph, graphlab)")
...
def init_cluster(conn, args):
    ...
    machine_names = ""
    for i in range(1, args.num_slaves+1):
        machine_names += '\n\\"%s%i\\"' % (args.cluster_name, i) 

    get_hosts = ('#!/bin/bash\n\n'
                 '# Set the prefix name and number of slaves/worker machines.\n'
                 '# Set the names of machines (an array, with master followed by workers)\n'
                 '# and the number of slaves/worker machines (excluding the master).\n'
                 '#\n'
                 '# NOTE: This file is automatically generated by uw-ec2.py init!\n\n'
                 '_MACHINES=(\\"\$(hostname)\\"%s)\n'
                 '_NUM_MACHINES=%d') % (machine_names, args.num_slaves)
